...
namespace ApiProjeCampWebUI.Controllers
{
    public class WhyChooseYummyController : Controller
    {
        ...
        public async Task<IActionResult> CreateWhyChooseYummy(CreateWhyChooseYummyDto createWhyChooseYummyDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7260/api/Services",
                stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7260/api/Services?id=" + id);
            return RedirectToAction("WhyChooseYummyList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7260/api/Services/GetService?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateWhyChooseYummy(UpdateWhyChooseYummyDto updateWhyChooseYummyDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7260/api/Services/", stringContent);
            return RedirectToAction("WhyChooseYummyList");
        }
    }
}
